...
%token <ptr> NUM ID READ WRITE IF WHILE EQ NE GT LT LE GE ASG PLUS MINUS DIV MUL MOD TRUE FALSE AND OR NOT MAIN LET
%token THEN ENDIF DO ENDWHILE DECL ENDDECL INTEGER BOOLEAN BEGINS END ELSE RETURN IN ENDLET
%type <ptr> GDecBlock FDefList Mainblock GDecList GDecl ArgType ArgList Arg FDef FDef_ArgList LDecBlock LDecList LDecl LIdList LId StmtList GIdList GId FType Type Stmt expr Id IdList NonEmptyExprList ExprList Main letAsgDecl letBody
...
LIdList : LId
	| LIdList ',' LId ;
LId : ID					{Linstall($1->NAME,var_type,lVariables++,1);};
...
Stmt: ID ASG expr';' 				{	
    ...
    | letAsgDecl letBody			{ 
							$$ = TreeCreate(VOID_TYPE,LET_NODETYPE,0,NULL,$1,$2,NULL,$1->leth);
							$1->Ptr3=NULL;
							isLet=0;
						};

letAsgDecl: LET'('Type ID ASG expr')' IN	{ 
							isLet = 1;
							allow = 1;
							Linstall($4->NAME,var_type,lVariables++,20000);
							allow = 0;
							$4->TYPE = var_type;
							if(typecheckLet($4,$6)){
								$$ = TreeCreate(INT_TYPE,ASG_NODETYPE,0,NULL,$4,$6,NULL,letHead);
							}else exit(0);
						};
letBody: BEGINS StmtList ENDLET			{
							$$ = $2;
...
ExprList : 			{$$ = NULL;}
	 | NonEmptyExprList	{$$ = $1;}; 
NonEmptyExprList: expr				{$$=TreeCreate(DUMMY_TYPE,DUMMY_NODETYPE,0,NULL,NULL,$1,NULL,NULL);}	
	 	| NonEmptyExprList ',' expr  	{$$=TreeCreate(DUMMY_TYPE,DUMMY_NODETYPE,0,NULL,$1,$3,NULL,NULL);};
%%
...
int typecheckLet(struct Tnode* id, struct Tnode* e){
	int crt_type =1;
						if(id->TYPE == INT_TYPE){
							if(e->TYPE == BOOL_TYPE) crt_type=0;
							else if(e->TYPE == VOID_TYPE){
								if(e->NODETYPE!=ID_NODETYPE && e->NODETYPE!=FUNCTION_NODETYPE)
										crt_type = 0;
								else if(getIDType(e->NAME) == BOOL_TYPE)
										crt_type = 0;
							}}
						else if(id->TYPE == BOOL_TYPE){
							if(e->TYPE == INT_TYPE) crt_type=0;
							else if(e->TYPE == VOID_TYPE){
								if(e->NODETYPE!=ID_NODETYPE && e->NODETYPE!=FUNCTION_NODETYPE)
									crt_type = 0;
								else if(getIDType(e->NAME) == INT_TYPE)
									crt_type = 0;
							}
							
						}
						if(!crt_type){printf("%d ERROR : Type Mismatch in '=' of LET\n",yylineno);exit(0);
						}else return 1;
}
...
void idLookUp(struct Tnode* ptr){
	if(isLet == 1){
		...
		while(t->PREV!=NULL && strcmp(t->NAME,ptr->NAME))
			t= t->PREV;
		if(t->PREV!=NULL && !strcmp(t->NAME,ptr->NAME)){		
			struct Lsymbol *entryl = Llookup(currentTable,ptr->NAME);
			ptr->Lentry = entryl;	
			return ;
		} 
