...
#define SWITCH_NODETYPE 27
#define CASE_NODETYPE 28
...
switch(Ptr1->TYPE){
	case INT_TYPE:
		switch(Ptr1->NODETYPE){
			...
			case MOD_NODETYPE: 
			case MUL_NODETYPE: 
						if(Ptr2->TYPE == INT_TYPE && Ptr3->TYPE == INT_TYPE) crt_type = 1;		
						else if(Ptr2->TYPE == BOOL_TYPE || Ptr3->TYPE == BOOL_TYPE) crt_type = 0;
						else{ 
	...
	case BOOL_TYPE:
			switch(Ptr1->NODETYPE){
				...
				case AND_NODETYPE:
				case OR_NODETYPE:
						if(Ptr2->TYPE == BOOL_TYPE && Ptr3->TYPE == BOOL_TYPE) crt_type = 1;
						else if(Ptr2->TYPE == INT_TYPE || Ptr3->TYPE == INT_TYPE) crt_type = 0;
						else{ 
							if(Ptr2->TYPE == VOID_TYPE){//Identifier&Function
								if(Ptr2->NODETYPE!=ID_NODETYPE && Ptr2->NODETYPE!=FUNCTION_NODETYPE) 										crt_type = 0;
								else if(getIDType(Ptr2->NAME) == INT_TYPE)
				...
				case NOT_NODETYPE:	
						...
						else if(Ptr2->TYPE == BOOL_TYPE) crt_type = 1;
						else{ 
							if(Ptr2->TYPE == VOID_TYPE){//Identifier&Function
								if(Ptr2->NODETYPE!=ID_NODETYPE && Ptr2->NODETYPE!=FUNCTION_NODETYPE) 										crt_type = 0;
								else if(getIDType(Ptr2->NAME) == INT_TYPE)
	...
	case VOID_TYPE:
			switch(Ptr1->NODETYPE){
				...
				case WHILE_NODETYPE:{
							...
							if(!crt_type){
								if(Ptr1->NODETYPE == IF_NODETYPE){
									if(Ptr1->VALUE == 1)
								     printf("%d ERROR : Type Mismatch in CONDITIONAL EXPRESSION\n",yylineno);
									else
										printf("%d ERROR : Type Mismatch in IF\n",yylineno);
								}
								else if(Ptr1->VALUE == 1)
									printf("%d ERROR : Type Mismatch in FOR\n",yylineno);
								else
				...
				case SWITCH_NODETYPE:{
							if(Ptr2->TYPE == BOOL_TYPE) crt_type = 0;
							else if(Ptr2->TYPE == INT_TYPE) crt_type = 1;
							else if(Ptr2->TYPE == VOID_TYPE){//Identifier&Function
								if(Ptr2->NODETYPE!=ID_NODETYPE && Ptr2->NODETYPE!=FUNCTION_NODETYPE) 										crt_type = 0;
								else if(getIDType(Ptr2->NAME) == INT_TYPE)
									crt_type = 1;
							...
							if(!crt_type){
								printf("%d ERROR : Type Mismatch in SWITCH\n",yylineno);
								exit(0);
							}else	return 1;
						}
				case READ_NODETYPE:{
