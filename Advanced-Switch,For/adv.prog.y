...
int AttachAtElse(struct Tnode* t, struct Tnode* e);
...
%token <ptr> NUM ID READ WRITE IF WHILE EQ NE GT LT LE GE ASG PLUS MINUS DIV MUL MOD TRUE FALSE AND OR NOT MAIN SWITCH 
%token THEN ENDIF DO ENDWHILE DECL ENDDECL INTEGER BOOLEAN BEGINS END ELSE RETURN CASE DEFAULT BREAK FOR
%type <ptr> GDecBlock FDefList Mainblock GDecList GDecl ArgType ArgList Arg FDef FDef_ArgList LDecBlock LDecList LDecl LIdList LId StmtList GIdList GId FType Type Stmt expr Id IdList NonEmptyExprList ExprList Main case caseStmts default
...
Stmt: ID ASG expr				{	
    ...
    | READ'('ID'['expr']'')'			{	arrayLookUp($3);
							...
							if(typecheck($1,$3,NULL) && validate_array_index($3,$5,entry)){
								$1->Ptr1 = $3;
								$1->Ptr2 = $5;
								$$ = $1;
							}
    ...
    | expr'?'Stmt':'Stmt			{
							$$ = TreeCreate(VOID_TYPE,IF_NODETYPE,1,NULL,$1,$3,$5);
							if(!typecheck($$,$1,NULL)) exit(0);
						}
    ...
    | FOR'('Stmt ';'expr ';' Stmt')''{' StmtList '}'	{
									...
									struct Tnode* tempw;
									struct Tnode* temps;
									tempb = TreeCreate(DUMMY_TYPE,DUMMY_NODETYPE,0,NULL,$10,$7,NULL);
									tempw = TreeCreate(VOID_TYPE,WHILE_NODETYPE,1,NULL,$5,tempb,NULL);
									if(typecheck(tempw,$5,NULL))
									    $$=TreeCreate(DUMMY_TYPE,DUMMY_NODETYPE,0,NULL,$3,tempw,NULL);
									else exit(0);
								}
    | SWITCH'('expr')''{' caseStmts default '}'		{ 	typecheck($1,$3,NULL);	
							   	AttachAtElse($6,$7);//Go till end and attach;
						    		$1->Ptr1 = $3; $1->Ptr2 = $6;
							};

caseStmts : case 				{ $$ = $1; }	
	  | caseStmts case 			{ $2->Ptr3 = $1; $$ = $2; };

case: CASE NUM':'StmtList BREAK';'	{$$=TreeCreate(VOID_TYPE,CASE_NODETYPE,0,NULL,$2,$4,NULL);};

default: DEFAULT':' StmtList BREAK ';' 	{$$=$3;};
