...
void Ginstall(char* NAME, int TYPE, int SIZE){ // Installation       ... ARGLIST no need now
	...
	if(SIZE>0){
		...
		Entry->TYPE = TYPE;
		Entry->SIZE = SIZE;
		Entry->BINDING = malloc(sizeof(int) * SIZE);
		Entry->NEXT = NULL;
		return ;
	}
	else{
	...
	if(Node!=NULL){
		switch(Node->TYPE){
			...
			case INT_TYPE:
				...
				case MINUS_NODETYPE: return(Evaluate(Node->Ptr1) - Evaluate(Node->Ptr2)); 
				...
				case ASG_NODETYPE: { struct Gsymbol *entry = Node->Ptr1->Gentry;
						     if(entry != NULL){
							if(Node->Ptr3 == NULL)
						     		*(entry->BINDING) = Evaluate(Node->Ptr2);
							else{
								r = Evaluate(Node->Ptr2);
								if(r>=0 && r<entry->SIZE)
									*(entry->BINDING+r) = Evaluate(Node->Ptr3);
								else{
									error = 1;
									printf("%d ERROR : Array \"%s\" out of bound\n",yylineno,Node->Ptr1->NAME);
									exit(0);
								}
							}
						     }
			...
			case BOOL_TYPE: 	
				...
				case LE_NODETYPE: return(Evaluate(Node->Ptr1) <= Evaluate(Node->Ptr2));
				case GE_NODETYPE: return(Evaluate(Node->Ptr1) >= Evaluate(Node->Ptr2));
				case EQ_NODETYPE: return(Evaluate(Node->Ptr1) == Evaluate(Node->Ptr2));
				case NE_NODETYPE: return(Evaluate(Node->Ptr1) != Evaluate(Node->Ptr2));
			...
			case VOID_TYPE:
			     switch(Node->NODETYPE){
				case READ_NODETYPE: {struct Gsymbol *entry = Node->Ptr1->Gentry;
						     if(entry != NULL){
						     	if(Node->Ptr2 == NULL){
								scanf("%d",entry->BINDING);
							}
							else{
								r = Evaluate(Node->Ptr2);
								if(r>=0 && r<entry->SIZE)
									scanf("%d",entry->BINDING+r);
								else{
									error = 1;
									printf("%d ERROR : Array \"%s\" out of bound\n",yylineno,Node->Ptr1->NAME);
									exit(0);
				...
				case WRITE_NODETYPE: {
							flag=1;
							int r = Evaluate(Node->Ptr1);
							if(flag)
								printf("%d\n",r);
						     }break;
				case IF_NODETYPE: if(Node->Ptr3 == NULL){ 
				...
				case WHILE_NODETYPE: while(Evaluate(Node->Ptr1))
						     ...
						     break;
				case ID_NODETYPE: {
							struct Gsymbol *entry = Node->Gentry;
							if(entry !=NULL){
								if(Node->Ptr1 == NULL){
...
switch(Ptr1->TYPE){
	...
	case BOOL_TYPE:
			switch(Ptr1->NODETYPE){
				...
				case AND_NODETYPE:
				case OR_NODETYPE:
						if(Ptr2->TYPE == INT_TYPE || Ptr3->TYPE == INT_TYPE){
								printf("%d %d ",Ptr2->TYPE,Ptr3->TYPE);	
								crt_type = 0;}
						else{ 
							if(Ptr2->TYPE == VOID_TYPE){
								if(Ptr2->NODETYPE != ID_NODETYPE)
